    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden gap-0 border-brand-charcoal/10">
                ...
                {/* Header Banner */}
                <div className={`p-6 pb-8 text-center relative overflow-hidden ${isSpecial ? "bg-[#1B2F45] text-white" : "bg-brand-charcoal/5"}`}>
                    {/* Decorative Pattern */}
                    {isSpecial && (
                    ...
                    <div className="relative z-10 space-y-2">
                        <div className={`text-xs font-mono uppercase tracking-widest opacity-70 ${isSpecial ? "text-brand-orange" : ""}`}>
                            Unlock Capacity
                        </div>
                        <h2 className="text-3xl font-bold tracking-tight">{planName}</h2>
                        <div className="flex items-baseline justify-center gap-1">
                            <span className={`text-lg font-light opacity-70`}>$</span>
                            <span className={`text-4xl font-black tracking-tighter ${isSpecial ? "text-brand-orange" : ""}`}>{price}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 space-y-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="email" className="text-xs font-mono uppercase tracking-widest opacity-70">Email Address</Label>
                            <Input
                                id="email"
                                type="email"
                        ...
                        <div className="space-y-3 pt-2">
                            <div className="text-xs font-mono uppercase tracking-widest opacity-50 mb-2">Included Features</div>
                            <ul className="grid grid-cols-1 gap-2">
                                {features.slice(0, 3).map((feat, i) => (
                                    <li key={i} className="flex items-start gap-2 text-sm text-brand-charcoal/80">
                                        <Check className={`w-4 h-4 mt-0.5 shrink-0 ${isSpecial ? "text-[#d97757]" : "text-brand-charcoal"}`} />
                                        <span>{feat}</span>
                                    </li>
                                ))}
                                {features.length > 3 && (
                    ...
                    <p className="text-[10px] text-center text-brand-charcoal/40 max-w-xs mx-auto leading-relaxed">
                        By submitting, you agree to receive follow-up communication regarding your account upgrade. No payment is processed at this time.
                    </p>
                </div>
