...
export function normalizeTechName(input: string): string | undefined {
    ...
    const partialMatch = TECH_ECOSYSTEM.find(
        t => normalized.includes(t.id) || normalized.includes(t.label.toLowerCase())
    );
    if (partialMatch) return partialMatch.id;

    // 5. Reverse partial (ID or label contained in input)
    for (const tech of TECH_ECOSYSTEM) {
        if (normalized.includes(tech.id) || tech.label.toLowerCase().split(' ').some(word => normalized.includes(word) && word.length > 3)) {
            return tech.id;
        }
    }
