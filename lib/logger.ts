...
class AppLogger {
  ...
  private formatMessage(
    ...
    const contextStr =
      Object.keys(mergedContext).length > (mergedContext.module ? 1 : 0)
  ...
  trace(message: string, context?: LogContext): void {
    if (process.env.NODE_ENV !== "production") {
      console.debug(
        `%cTRACE%c ${this.formatMessage("trace", message, context)}`,
        "color: #888",
        "color: inherit",
      );
    }
  ...
  debug(message: string, context?: LogContext): void {
    if (process.env.NODE_ENV !== "production") {
      console.debug(
        `%cDEBUG%c ${this.formatMessage("debug", message, context)}`,
        "color: #007acc",
        "color: inherit",
      );
    }
  ...
  info(message: string, context?: LogContext): void {
    console.log(
      `%cINFO%c ${this.formatMessage("info", message, context)}`,
      "color: #28a745",
      "color: inherit",
    );
  }
  ...
  warn(message: string, context?: LogContext): void {
    console.warn(
      `%cWARN%c ${this.formatMessage("warn", message, context)}`,
      "color: #ffc107",
      "color: inherit",
    );
  }
