...
Generated by [Simulark](https://simulark.app). Compatible with the [Agent Skills](https://agentskills.io) open standard and \`npx skills\` ([vercel-labs/skills](https://github.com/vercel-labs/skills)).

## Quick install (recommended)

Extract this folder, then from your project root:

\`\`\`bash
npx skills add ./${skillName} --skill ${skillName} -a cursor -y
\`\`\`

Global install (all projects):

\`\`\`bash
npx skills add ./${skillName} --skill ${skillName} -a cursor -g -y
\`\`\`

If you extracted into \`.agents/skills/\`:

\`\`\`bash
npx skills add . --skill ${skillName} -a cursor -y
\`\`\`

## Package contents

- \`SKILL.md\` — agent instructions (Level 1 + 2)
- \`references/\` — architecture snapshot, Mermaid diagram, service catalog (Level 3)

## Other agents

Replace \`-a cursor\` with your agent: \`claude-code\`, \`codex\`, \`opencode\`, etc. Run \`npx skills add --help\` for the full list.
`;
}

export function buildSkillInstallCommands(skillName: string): {
  project: string;
  global: string;
  list: string;
} {
  return {
    project: `npx skills add ./${skillName} --skill ${skillName} -a cursor -y`,
    global: `npx skills add ./${skillName} --skill ${skillName} -a cursor -g -y`,
    list: `npx skills add ./${skillName} --list`,
  };
}
...
export async function packageSkill(skill: GeneratedSkill): Promise<Blob> {
  ...
  // Cursor / universal agent path (vercel-labs/skills discovery)
  zip.file(`.agents/skills/${dir}/SKILL.md`, skill.skillMd);
  for (const [path, content] of Object.entries(skill.files)) {
    if (path === "README.md") continue;
    zip.file(`.agents/skills/${dir}/${path}`, content);
  }
