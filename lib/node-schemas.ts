...
export const FunctionSchema = v.object({
    ...BaseSchema.entries,
    runtime: v.picklist(['nodejs18.x', 'nodejs20.x', 'python3.11', 'go1.x', 'java17']),
    memory: v.number(),
    timeout: v.number(),
    handler: v.string(),
});
...
export const DEFAULT_VALUES: Record<string, any> = {
    function: { runtime: 'nodejs18.x', memory: 128, timeout: 30, handler: 'index.handler' },
    database: { engine: 'postgres', version: '15', storage_gb: 20, instance_type: 'db.t3.micro' },
