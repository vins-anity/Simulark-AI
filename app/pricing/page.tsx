        ...
        <div className="max-w-6xl mx-auto space-y-10">
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-xl text-brand-gray-mid max-w-2xl mx-auto font-light leading-relaxed">
              Select the computational capacity required for your infrastructure
              engineering.
            </p>
          </div>
          ...
          <div className="max-w-3xl mx-auto pt-10 border-t border-brand-charcoal/10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <h3 className="font-mono text-sm uppercase tracking-wider opacity-60">
                  Enterprise & Teams
                </h3>
                <p className="text-sm text-brand-gray-mid leading-relaxed">
                  Need custom SLAs, SSO, or on-premise deployment? We offer
                  bespoke contracts for large engineering teams.
                </p>
                <a
                  href="#"
                  className="inline-block mt-2 text-sm font-medium border-b border-brand-charcoal pb-0.5 hover:opacity-70 transition-opacity"
                >
                  Contact Sales -&gt;
                </a>
              </div>
              <div className="space-y-2">
                ...
                <p className="text-sm text-brand-gray-mid leading-relaxed">
                  Simulark contributes to the open ecosystem. Sketch and
                  Blueprint tiers directly fund our R&D in generative
                  architecture.
                </p>
              </div>
            </div>
...
function PricingCard({
  ...
  refCode: string;
  btnText: string;
  variant?: "default" | "premium" | "special";
  tag?: string;
}) {
  ...
  return (
    ...
    >
      {/* Tech Markers */}
      <div
        className={`absolute top-0 left-0 w-3 h-3 border-t border-l ${isSpecial ? "border-[#d97757]/40" : "border-brand-charcoal/20"}`}
      />
      <div
        className={`absolute bottom-0 right-0 w-3 h-3 border-b border-r ${isSpecial ? "border-[#d97757]/40" : "border-brand-charcoal/20"}`}
      />
      ...
      <div className="relative z-10 space-y-4 flex-1 flex flex-col">
        <div className="space-y-1">
          <h3
            className={`text-xl font-bold font-mono tracking-tight ${isSpecial ? "text-[#d97757]" : ""}`}
          >
            {name}
          </h3>
          <p
            className={`text-[12px] italic font-light ${isSpecial ? "text-white/70" : "text-brand-gray-mid"}`}
          >
            {desc}
          </p>
        </div>
        ...
        <div className="flex items-baseline gap-1">
          <span
            className={`text-2xl font-light ${isSpecial ? "text-[#d97757]" : ""}`}
          >
            $
          </span>
          <span
            className={`text-5xl font-black tracking-tighter ${isSpecial ? "text-[#d97757]" : ""}`}
          >
            {price}
          </span>
          {period && (
            <span
              className={`text-[10px] font-mono opacity-50 ${isSpecial ? "text-white" : "text-brand-charcoal"}`}
            >
              {period}
            </span>
          )}
        ...
        <div
          className={`h-px w-full ${isSpecial ? "bg-white/10" : "bg-brand-charcoal/5"}`}
        />
        ...
        <ul className="space-y-2 py-1 flex-1">
          {features.map((feat, i) => (
            <li key={i} className="flex items-start gap-2 text-[13px]">
              <Check
                className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${isSpecial ? "text-[#d97757]" : "text-brand-charcoal/40"}`}
              />
              <span
                className={isSpecial ? "opacity-90 font-medium" : "opacity-80"}
              >
                {feat}
              </span>
            </li>
        ...
        <div className="pt-4 mt-auto">
          ...
          <div
            className={`mt-3 text-center font-mono text-[8px] uppercase tracking-widest opacity-30 ${isSpecial ? "text-white" : ""}`}
          >
            REF: {refCode}
          </div>
        </div>
